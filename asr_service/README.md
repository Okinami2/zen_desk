...
4. **日志丢失与缓冲坑 (已修复)**：之前使用脚本后台启动服务时，发现 `asr_service` 没有任何日志输出。原因系 Linux 的 C 标准库对非 TTY 输出开启了块缓冲 (Block Buffering)。已通过在 `logger.h` 宏中全局补全 `fflush(stdout);` 解决。
5. **台灯高频闪烁与指令封印 (已修复)**：
   - 之前台灯刺眼闪烁是由于 PWM 底层每秒33次的无脑覆盖写入导致驱动重置。已加入**脏数据过滤器** (`g_last_cold_duty`) 完美治愈。
   - “打开台灯”等手动控制曾被“封印”，是因为刚打开灯就在同一循环内被系统的闲置状态**强行重置**。现已修正状态广播鉴权，手动指令再也不会被覆盖。
   - 冷暖色温与指令相反，证实为**物理 PWM 引脚（冷暖色）接反**，现已在代码中软性将 0/1 通道互换，恢复正常。
...
虽然语音模块到底层硬件（台灯、雷达、串口）的后端链路已经**100% 畅通**，但后续仍有以下重点工作需要开展：

1. **新版 Qt 客户端的上板编译**：
   - 目标：将我们写好的带有 `QUdpSocket` 监听功能的 `qt_client` 源码，使用交叉编译工具链（如 `aarch64-linux-gnu`）编译出可运行在海鸥派上的二进制文件。
   - 预期效果：替换掉板子上的老旧 UI 后，喊“打开看板”将直接切换屏幕页面，喊“唤醒”屏幕会弹出🎤麦克风悬浮图标。
2. **深层业务逻辑的精细打磨**：
   - 根据后续真实使用体验，可能需要继续微调**雷达与语音的优先级冲突**（例如：离座多久后才覆盖专注状态，或者调亮台灯后是否锁定亮度不被专注状态覆盖）。
   - 可考虑在 `device_service` 的 `device_adjust_lamp_brightness` 中加入非线性亮度调节映射（类似人眼的感知曲线），让 20% 的亮度调整更加柔和自然。
